...
## Nawigacja — floating pill o STAŁYM rozmiarze, animuje się tylko szkło
...
**Rozmiar pilla jest STAŁY na całej stronie — dokładnie taki jak w sekcji hero (usunięte 15.09.2026 na życzenie; wcześniej pill fizycznie kurczył się po ~60px scrolla).** Wysokość 80px (mobile 68px), padding 22px (mobile 14px), logo 42px (mobile 34px), CTA 52px (mobile 44px) — te same wartości od góry strony aż do samego dołu, niezależnie od tego, ile przescrollowano. Zero animacji rozmiaru, zero klasy `.scrolled` (usunięta razem z całym mechanizmem — patrz „Uwagi przy zmianach").

**Jedyne, co się animuje, to SZKŁO — osobna oś stanu, sterowana klasą `.nav--solid`.**

| | stan hero (`--nav-glass-p: 0`) | stan solid (`--nav-glass-p: 1`) |
|---|---|---|
| próg | — | dolna krawędź **hero** minie `navMidY = 60` (histereza 40px) |
| tło | `0.30` (wariant `nav--light`: `0.22`; podstrony: `0.94`, patrz „Nawigacja nad hero podstron") | `0.86` (mobile solid 0.88) |
| blur + saturate | `16px / 160%` | `18px / 180%` |
| obrys | `0.09` (`nav--light`: biały `0.20`; podstrony: `0.10`) | `0.07` (`nav--light`: `0.14`) |
| inset highlight | `0.55` (`nav--light`: `0.18`; podstrony: `0.45`) | `0.30` (`nav--light`: `0.12`) |
| cień | `0.05` (`nav--light`: `0.16`; podstrony: `0.09`) | `0.12` (`nav--light`: `0.22`) |

**W hero pill ma glassmorphism, nie jest przezroczysty** (tak było przez chwilę 01.09.2026 — porzucone): **mocny blur już w stanie hero** (16px) przy **niskim ticie**, plus **świetlisty hairline** i **wewnętrzny highlight** na górnej krawędzi. To ten blur, nie tło, robi tu efekt szkła. **Gęstnieje dopiero po opuszczeniu sekcji hero**, w tym samym momencie co przełączenie `nav--light` (ten sam `navMidY`), więc gęstość szkła i kolor linków zmieniają się razem. Hero każdej strony JS znajduje przez `document.querySelector('#hero, #svc-hero, #me-hero')`.

**Jak to działa (`style.css` sekcja „Navigation — floating pill o STAŁYM rozmiarze" + blok nawigacji w JS każdej strony):**

- Rozmiar to **stałe custom properties na `#nav`** (`--nav-h`, `--nav-px`, `--nav-logo`, `--nav-cta-h`, `--nav-link-px`), zdefiniowane raz w CSS i **nigdy nie nadpisywane przez JS** — używane w CSS przez `calc(var(--nav-h) * 1px)`.
- **Szkło animuje JEDNA liczba — `--nav-glass-p` (0 = hero, 1 = po hero).** Reszta to *wartości brzegowe* obu stanów w CSS: `--nav-a-hero/-solid` (alpha tła), `--nav-line-a-hero/-solid` (obrys), `--nav-hl-a-hero/-solid` (inset highlight), `--nav-sh-a-hero/-solid` (cień); blur i saturate liczone wprost z `--nav-glass-p`. `.nav-bar` miesza je czterema `calc()`-ami (`--nav-mix-a`, `--nav-mix-line`, `--nav-mix-hl`, `--nav-mix-sh`). **Skutek: wariant `nav--light` (i override podstron, patrz niżej) ma własne brzegi bez dotykania JS** — inaczej ciemny tint musiałby interpolować RGB przez szarość i przy zmianie stanu mignąłby brudnym kolorem. Kolor obrysu jest rozbity na `--nav-line` (rgb) + alpha; dawne `--nav-border`, `--nav-alpha`, `--nav-shadow` i `--nav-blur` **już nie istnieją**.
- **Dodając nowy wariant kolorystyczny nav-a nadpisujesz tylko pary `*-hero` / `*-solid` na `#nav.twoja-klasa`** — JS zostaje nietknięty.
- **Jeden tween GSAP** na obiekcie `navGlass { g }` (`paintNavGlass`), ease `power2.out`, `duration 0.45`, bez naddatku — overshoot na tle/blurze wyglądałby jak błąd renderowania. Każde przejście tworzy nowy tween (poprzedni `kill()`). `g` jest dodatkowo klamrowane do `[0, 1]`, bo `--nav-glass-p` poza tym zakresem dałby ujemną alphę.
- Próg ma **histerezę**: włącza się gdy dół hero ≤ 60px i gaśnie dopiero powyżej 100px. Detekcja siedzi w `updateNav` (ten sam listener co `nav--light`) — celowo bez ScrollTrigger, żeby nav nie zależał od pluginu i działał identycznie na wszystkich 5 stronach.
- Gdy tweenem steruje GSAP, `#nav` dostaje klasę **`.nav--gsap`**, która **wyłącza CSS-owe `transition`** na `.nav-bar` (inaczej transition goniłoby tween).
- **Fallback bez GSAP / przy `prefers-reduced-motion`:** JS nie wpisuje inline zmiennej; gęstość szkła ustawia sama klasa `.nav--solid`, a przejście dowozi `transition` na `.nav-bar` zwykłym `ease`. Przy `prefers-reduced-motion` transition jest dodatkowo zerowany.
- **Czytelność w stanie hero:** tint 0.22–0.30 (na `index.html`) dalej przepuszcza tło sekcji, więc `#nav:not(.nav--solid)` dokłada delikatny `text-shadow` na logo i linki — jasną poświatę w wariancie domyślnym (pod „Kontakt" przewija się ciemny mockup telefonu), ciemną w `nav--light`. Na podstronach, gdzie tint jest wysoki (0.94), poświata i tak jest niepotrzebna, ale nieszkodliwa.
...
- `.btn-nav-brand` ma **stałą wysokość** z `--nav-cta-h` (padding pionowy usunięty), a kółko ikony `--nav-cta-h − 14`.
...
- Zmiana wysokości/paddingu pilla wymaga edycji **jednego miejsca**: `#nav` w `style.css` (desktop) + media query `max-width: 767px` (mobile) — te wartości nie istnieją już w JS, więc nie trzeba ich synchronizować z 5 plikami HTML.
- Zmiana wartości szkła wymaga edycji **dwóch miejsc**: tokenów `--nav-a-hero` itd. na `#nav`/`#nav.nav--light` w CSS i (jeśli zmienia się czas/ease tweena) `glassTween` w JS **każdej z 5 stron**. Cały blok nav-JS jest **bajt w bajt identyczny we wszystkich 5 plikach** — zmieniaj go skryptem, nie ręcznie.
...
- **Ścieżki root-relative:** `img/…`, `style.css`, logo `index.html`. Podstrony są w katalogu głównym, więc bez `../`. **Logo (nav + footer) NIE linkuje na `/`** — repo jest hostowane jako GitHub Pages project page (`stecudesign.github.io/lokalne-www/`), więc `href="/"` prowadzi do roota domeny (404), nie do repo. `href="index.html"` działa identycznie lokalnie i na Pages.
...
- **`darkSectionIds` w nav-JS jest per-strona:** lista id ciemnych sekcji, nad którymi nav ma białe linki (`nav--light`). Ustaw ją pod realne ciemne pasy danej podstrony (na `index.html`: `['o-mnie-v2','realizacje']`; na `oferta.html`: `['svc-strony','svc-design','svc-opieka']`). Nowa podstrona bez ciemnych sekcji → pusta tablica `[]`. **Hero (`svc-hero`/`me-hero`) celowo NIE jest w tej liście na żadnej podstronie od 15.09.2026**, mimo że jego tło jest niebieskie — patrz „Nawigacja nad hero podstron" niżej.
...
- **Scrollspy nie istnieje** — nawigacja podświetla wyłącznie aktywną **stronę** (patrz punkt wyżej), nie sekcję. `updateNav` odpowiada tylko za `nav--solid` (gęstość szkła) i `nav--light`.
...
1. `#svc-hero .svc-hero.svc-hero--b.svc-hero--case` — **hero „case study" na pełnym niebieskim tle** (wariant wspólny dla **wszystkich podstron**: `oferta.html`, `proces.html`, `kontakt.html` i `omnie.html` — patrz sekcje tych stron; jasny wariant `--b` nie jest już nigdzie używany). **Proporcje wzorowane na `marceldigital.com/case-studies`** (zmierzone w DevTools: H1 102px, kolumny 579/709 px, mockup wychodzący poza kontener, karta wcięta pod mockupem). Layout 2 kolumny (grid `55fr / 45fr`, gap `clamp(2.5rem, 4.5vw, 4rem)`, `align-items: center`; na tablecie `48fr / 52fr`, żeby karta nie robiła się wąska): lewo badge + H1 + lead, prawo `.svc-hero-media` = pierścień + mockup + karta realizacji. Padding sekcji `17.5rem 0 12rem` (tablet `14rem / 8.75rem`, mobile `11rem / 8.5rem` + `gap: 6rem` w kolumnie). **Te wartości to nie "ładne liczby", tylko wynik celu "hero +20% wysokości"** — wysokość sekcji wyznacza kolumna medialna (na 1440: media 598px vs tekst ~247px), więc wzrost trzeba było dołożyć w paddingu. Zmierzone: 894→1070 (+19.7%), 656→788 (+20.1%), 888→1068 (+20.3%). Zmieniając padding, przelicz od nowa — sam `rem` nic nie powie.
   ...
   - **Nav:** hero jest niebieski, ale `'svc-hero'` **nie jest** w `darkSectionIds` — nad hero nav ma jasny wariant, tak samo jak na `index.html` (patrz „Nawigacja nad hero podstron" w sekcji Nawigacja). `darkSectionIds` obejmuje tylko realne ciemne pasy poniżej.
...
**Nav JS:** `updateNav` wymusza `nav--light` gdy scroll nad pasem ciemnym — `darkSectionIds = ['svc-strony','svc-design','svc-opieka']` (bez `'svc-hero'` — patrz „Nawigacja nad hero podstron"). Akordeon: klik na `.svc-acc-head` toggluje `.is-open` (zamyka pozostałe w tym samym `.svc-accordion`).
>
...
2. `#me-mission .me-mission` — `.me-mission-top` (grid: `.me-eyebrow` „● O MNIE" + `.me-mission-text` duży akapit z `<strong>` w kolorze navy). Niżej `.me-mission-lower` — jedno szerokie zdjęcie 16:9 `.me-mission-photo.me-mission-photo--wide` `img/omnie-koduje.webp` (kodowanie strony w domowym biurze, klimat jak referencja). **Karta statystyki `.me-statcard` (+20% z mini wykresem `.me-bars`) usunięta z HTML — klasy `.me-statcard*`/`.me-bar*` pozostały w `style.css` jako martwy kod.**
   - **Eyebrow „O MNIE" usunięty (18.09.2026).** Klasa `.me-eyebrow` **zostaje** w `style.css` — używa jej jeszcze `#me-why` („DLACZEGO JA"). Po usunięciu spanę akapit został jedynym dzieckiem `.me-mission-top`, więc ma teraz jawne `grid-column: 2` — bez tego auto-placement wrzuciłby go do wąskiej pierwszej kolumny (`0.7fr`). **Oba breakpointy (`max-width: 767px` i `768–1023px`) przestawiają grid na 1 kolumnę i MUSZĄ resetować to na `grid-column: 1`**, inaczej powstaje kolumna implicit i tekst ściska się do prawej. Lewa kolumna jest teraz pusta — to celowe, trzyma editorialne wcięcie akapitu.
   - **Reveal wipe: słowa rozjaśniają się kolejno wraz ze scrollem.** Osobny IIFE w `omnie.html` dzieli akapit na słowa i animuje ich `opacity` `0.12 → 1` przez `ScrollTrigger` ze `scrub: 0.4` (`start: top 78%`, `end: bottom 20%`). `duration: 1.2` przy `stagger: 0.4` daje nakładanie się — kilka słów rozjaśnia się naraz, więc czyta się to jak fala, nie jak zapalanie pojedynczych żarówek. Zmierzone na 1440: 696px scrolla na 31 słów (~22px na słowo). Wcześniejsze `end: bottom 55%` dawało tylko 388px i efekt był zbyt gwałtowny.
   - **Podział na słowa idzie po węzłach tekstowych (`TreeWalker`), nie po `innerHTML`** — w akapicie są `<strong>`, które muszą przetrwać. `split(/(s+)/)` z grupą przechwytującą zachowuje spacje jako osobne węzły, więc łamanie wierszy nie zmienia się ani o piksel. Sprawdzone: `textContent` po podziale jest identyczny z oryginalnym, a `<strong>` mają w środku własne `.rw-word`.
   - **Stan początkowy (przygaszone słowa) celowo NIE siedzi w CSS.** Gdyby CDN z GSAP nie odpowiedział, akapit zostałby na zawsze wyszarzony. Bez GSAP albo przy `prefers-reduced-motion` IIFE robi wczesny `return` i **w ogóle nie dzieli tekstu** — zweryfikowane na kopii strony bez GSAP: zero `.rw-word`, `opacity: 1`. To odwrotność konwencji `.gsap-enhance` używanej gdzie indziej w projekcie i jest to świadome.
   - `.fade-up` zdjęte z `.me-mission-top` — reveal słów przejął animację wejścia tego bloku, dwie na raz robiły mętny efekt.
...
4. `#me-approach .me-sol` — `.me-sol-h2` + `.me-sol-grid` (lewo `.me-sol-photo` `img/omnie_kodowanie.png`, kadr `object-position: 50% 50%`; prawo `.me-sol-list` `<ol>` z `.me-sol-row` = `.me-sol-num` + `.me-sol-title` + `.me-sol-arrow`; wiersz `.me-sol-row--active` wyróżniony na niebiesko, hover przesuwa strzałkę i wcięcie).
   - **Lista równa się do zdjęcia górą I dołem (18.09.2026).** Wcześniej `.me-sol-grid` miał `align-items: end`, więc lista była dosunięta do dołu i zaczynała się ~179px poniżej górnej krawędzi zdjęcia. Teraz `align-items: stretch`, `.me-sol-list` jest kolumną flex, a `.me-sol-row` ma `flex: 1` — wiersze dzielą wysokość zdjęcia po równo (zmierzone na 1440: 5 × ~106px = 528.6px, różnica góra/dół 0px).
   - **Wysokość wiersza narzuca ZDJĘCIE, nie tekst.** Wysokość rzędu gridu bierze się z proporcji `omnie_kodowanie.png` (atrybuty `width`/`height` w HTML → aspect 2244/2804), bo intrinsic wysokość listy jest od niej niższa. Gdybyś mocno podbił `font-size`/`padding` w wierszach albo podmienił zdjęcie na szersze, lista może przerośnąć zdjęcie — wówczas to ona zacznie rozpychać rząd, a zdjęcie się rozciągnie (`object-fit: cover` przytnie kadr). Sprawdź sumę wysokości wierszy, zanim to zmienisz.
   - `padding: 1.4rem` na wierszu zostaje jako **minimum dla mobile**, gdzie grid jest 1-kolumnowy: `flex: 1` nie ma tam czego rozdzielać (kontener ma auto-wysokość), więc wiersze wracają do wysokości treści i łamią się na 2 linie — to celowe, nie regres.
   - **Wyróżnienie wędruje po liście wraz ze scrollem (18.09.2026).** `.me-sol-row--active` nie jest już przypisane na stałe do jednego punktu — osobny IIFE w `omnie.html` (po bloku Lenis) tworzy `ScrollTrigger` na `.me-sol-grid` (`start: top 75%`, `end: bottom 45%`) i w `onUpdate` przelicza `self.progress` na indeks wiersza. Zmierzone na 1440: ~798px scrolla na 5 punktów, czyli ok. 160px na punkt; sekwencja 1→2→3→4→5 bez przeskoków.
   - **Dlaczego scroll, a nie pętla `setInterval`:** lista jest do czytania we własnym tempie. Ruch niezależny od użytkownika ciągnie wzrok, czyta się jak karuzela, która nic nie przełącza, i bije się ze stanem `:hover` (dwa wiersze wyglądające na aktywne naraz). Sterowanie scrollem zatrzymuje się, gdy użytkownik przestaje scrollować.
   - **Fallback jest w HTML, nie w JS:** `me-sol-row--active` stoi na **01** w markupie. Gdy brak GSAP albo `prefers-reduced-motion: reduce`, IIFE robi wczesny `return` i nie dotyka DOM — zostaje statyczne wyróżnienie pierwszego punktu (zweryfikowane na kopii strony bez GSAP). Wcześniej podświetlone było **02**, arbitralnie, bez uzasadnienia w treści.
   - `onLeave` / `onLeaveBack` dopinają skrajne stany — `onUpdate` nie odpala się poza zakresem trigger'a, więc bez nich szybkie przewinięcie przez sekcję zostawiałoby wyróżnienie w przypadkowym miejscu.
   - `setActive` ma guard `if (i === current) return` — bez niego klasa byłaby przepisywana co klatkę scrolla, co restartowałoby `transition: color`.
   - `.me-sol-num` dostał `transition: color` (wcześniej nie miał, bo stan był statyczny). `font-variant-numeric: tabular-nums` jest tu teraz **konieczny**, nie kosmetyczny — bez niego numer zmieniałby szerokość przy każdym przeskoku.
   - `--active` zmienia `font-weight` na 700, ale **nie powoduje przelewu tekstu**: `.me-sol-title` ma `flex: 1`, więc szerokość boksu nie zależy od grubości pisma. Sprawdzone na 1440 i 801px — układ linii jest identyczny niezależnie od tego, który wiersz jest aktywny.
   - Typografia podbita razem z wysokością: `.me-sol-title` `1.0625rem → 1.25rem` + `line-height: 1.4`, `.me-sol-num` `0.95rem → 1.0625rem` (i `width` `2.5rem → 2.9rem`, żeby numer nie ścisnął się przy większym stopniu pisma).
5. `.svc-cta` (reuse z oferty) + `.footer-section`.

**Animacje:** `.fade-up` (IntersectionObserver) + jeden `ScrollTrigger` na liście `#me-approach` (patrz punkt 4 wyżej). GSAP, ScrollTrigger i Lenis są na tej stronie załadowane.
